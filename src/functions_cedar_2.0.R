cedar <- function(Y_raw,  # bulk observed data row:feature, col:sample
                  ...
                  # The dimnension must same as tree input.
  ...
  if( is.null(tree) ){
    tree.input <- tree.est(pval, de.res, similarityFun = similarity.function)
  }else{
    tree.type <- 'custom'
    tree.input <- list()
...
tree.est <- function(pval, de.res, similarityFun = NULL){
