...
### other parameters 
cell.type <- c('Neutrophil','Monocyte')
...
tree.options <- list('tt_tp' = rbind(c(1,1),
                                     c(1,2)),
                     'et_ep' = NULL)

prior.prob.options <- list('tt_tp' = rbind(c(rep(0.2,2)),
                                           c(rep(0.5,2))),
                           'et_ep' = NULL)


tree.num <- length(tree.options)
colnames(tree.options[['tt_tp']])  <- cell.type

data.noise.options <- c(0.01, 0.1, 1, 2)
sim.scenarios <- paste0('data_noise_two_cells_',data.noise.options)
...
for( data.noise.ix in data.noise.options){
  ...
  sim_process(sim.seed = 12345, 
              ...
              Sample.N = 100, 
              cell.type = cell.type, 
              dirichlet.alpha = c(27.94, 3.10), 
              main.path = main_path, 
              output.sub.path = output_sub_path,
              output.file.name = paste0("/data_noise_two_cells_",
                                        data.noise.ix,".rda"),
              tree.input = rbind(c(1,1),
                                 c(1,2)), 
              p.matrix = rbind(c(rep(0.2,2)),
                               c(rep(0.5,2))), 
              cutoff.fdr = NULL, 
              cutoff.pval = 0.01, 
...
write.csv(round((auc_rocr_all),digits = 3), file = paste0(data_path,'/auc_rocr_2_cells','.csv'))
write.csv(round((auc_pr_all),digits = 3), file = paste0(data_path,'/auc_pr_2_cells','.csv'))
...
write.csv(round((summary_mcc),digits = 3), file = paste0(data_path,'/mcc_2_cells','.csv'))
write.csv(round((summary_fdr),digits = 3), file = paste0(data_path,'/fdr_2_cells','.csv'))
