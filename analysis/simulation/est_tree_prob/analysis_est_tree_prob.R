...
png(paste0(data_path,'/figS_PR.png'),height=15, width=20, res=480,unit='in')
...
### MCC curve
png(paste0(data_path,'/figS_MCC.png'),height=15, width=20, res=480,unit='in')
...
for(ss.ix in sim.scenarios){
  ...
  for(cell.ix in 1:cell.num ){
    ...
    boxplot(data.frame(mcc.obs_res$sample_size_100$true[,cell.ix,]), col = colors ,
            main =  paste0('Cell type: ', cell.ix,'\n', 
                           paste0( 'Sample size: ', unlist(strsplit(ss.ix, '_'))[3] ) ),
            ylab = 'MCC',
            cex.lab = 1.5,
...
### TDR curve
png(paste0(data_path,'/figS_TDR.png'),height=15, width=20, res=480,unit='in')
par(mfrow=c(2,3))
for( ss.ix in sim.scenarios){
  for( cell.ix in 1:cell.num){
    for(tree.ix in 1:tree.num){
