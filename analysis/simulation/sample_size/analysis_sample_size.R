...
#png(paste0(output_path,'/fig3_PR_',ss.ix,'.png'),height=6,width=9,res=480,unit='in')
png(paste0(data_path,'/fig3_PR_all','.png'),height=9,width=18,res=480,unit='in')
par(mfrow=c(3,6))
for( ss.ix in sim.scenarios){
  for( cell.ix in 1:cell.num){
    for(method.ix in 1:method.num){
      ...
      if(method.ix == 1){
        plot(perf.tmp, avg = 'threshold', downsampling = 1, type = 'l', lwd = 3,
             ...
             main = paste0(ss.ix,'\n','Cell type: ',cell.ix), col=col.tmp[method.ix],
             lty = 1)
      }else{
        plot(perf.tmp, avg = 'threshold', downsampling = 1, type = 'l', lwd = 3,
             col=col.tmp[method.ix], lty = 1, add=T)
      }
    }
    
    if(cell.ix ==1){
      legend('bottomleft',legend = methods,col = col.tmp,
             lty= 1,bty = 'n',lwd=2 )
    }
...
colors <- col.tmp
#png(paste0(output_path,'/fig3_MCC_',ss.ix,'.png'),height=6,width=9,res=480,unit='in')
png(paste0(data_path,'/fig3_MCC','.png'),height=9,width=18,res=480,unit='in')
par(mfrow=c(3,6))
for(ss.ix in sim.scenarios){
  ...
  for(celltype.ix in 1:cell.num ){
    ...
    boxplot(data.frame(mcc.obs_res[[ss.ix]][['true']][,celltype.ix,]), col = colors ,
            main = paste0(ss.ix,'\n','Cell type: ', celltype.ix ),
            ylab = 'MCC',
            cex.lab = 1.5,
            cex.main = 2, xaxt = "n",
...
png(paste0(data_path,'/fig3_TDR','.png'),height=9,width=18,res=480,unit='in')
par(mfrow=c(3,6))
for( ss.ix in sim.scenarios){
  for( cell.ix in 1:cell.num){
    for(method.ix in 1:method.num){
      if(method.ix == 1){
        ylim.min <- min(rank.accuracy_avg.res[[ss.ix]][['true']])
        plot(rank.accuracy_avg.res[[ss.ix]][['true']][,cell.ix,method.ix]~toprank, 
             type = 'l', lwd = 3, cex.main= 2, cex.lab=1.5, 
             main = paste0(ss.ix,'\n','Cell type: ',cell.ix), 
             col=col.tmp[method.ix], lty = lty.tmp[method.ix], ylim=c(ylim.min,1),
             ylab='true sites %', xlab= '# top ranked sites')
      }else{
        lines(rank.accuracy_avg.res[[ss.ix]][['true']][,cell.ix,method.ix]~toprank,
              type = 'l', lwd = 3, col=col.tmp[method.ix], lty = lty.tmp[method.ix])
      }
    }
