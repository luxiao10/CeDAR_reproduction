...
png(paste0(data_path,'/figS_TDR','.png'),height=9,width=18,res=480,unit='in')
par(mfrow=c(2,3))
for( cell.ix in 1:cell.num){
  for( prop.ix in prop.type){
    ...
    for(method.ix in 1:method.num){
      ...
      if(method.ix == 1 & prop.ix =='true'){
        plot(rank.accuracy_avg.res[[prop.ix]][,cell.ix,method.ix]~toprank, 
             type = 'l', lwd = 3, cex.main= 2, cex.lab=1.5, 
             main = paste0('Cell type: ',cell.ix), col=col.tmp[method.ix],
             lty = lty.tmp[method.ix], ylab = 'true sites %', 
        ...
        lines(rank.accuracy_avg.res[[prop.ix]][,cell.ix,method.ix]~toprank,
...
png(paste0(data_path,'/figS_sim_est_prop_PR_MCC.png'), height = 3.5*3, width = 3.5*7, 
    unit='in', res = 480)
par(mfrow=c(2,6), omi=c(0.3,0.3,0.3,0.3))
methods <-      c( 'toast',  'tca',    'cs_sam',  'celldmc','cedar_s','cedar_m')
col.tmp <- rep( c( '#8DA0CB','#FFD92F',"#B3B3B3", "#66C2A5",'#A6D854','#E78AC3'), 1)
...
for( cell.ix in 1:cell.num){
  ...
  if(cell.ix == 1){
    legend('bottomleft', legend= paste0( rep(c('TOAST','TCA','csSAM','CellDMC' ,'CeDAR-S', 'CeDAR-M'), 2 ),
                                         rep(c(' + true prop', ' + est prop'), each =6) ),,
           col = col.tmp, lty = c(rep(1,6),rep(3,6)), border = NA, lwd=3, cex= 1.1, bty='n')
  }
}
...
colors <- col.tmp
for(cell.ix in 1:cell.num ){
  data.tmp <- data.frame(cbind(mcc.obs_res[['true']][,cell.ix,],
                               mcc.obs_res[['estimated']][,cell.ix,]))
  boxplot(data.tmp, col = col.tmp ,
          ylab = 'MCC', cex = 3, xaxt = "n" ,
          ylim = c(0, 1),
          yaxis.cex.axis=3,cex.lab=1.2)
  axis(side = 1, labels = FALSE)
