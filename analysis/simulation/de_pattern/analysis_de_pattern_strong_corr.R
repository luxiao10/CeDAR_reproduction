...
de.patten.options <- c('strongcorr_indep_all', 'strongcorr_corr_single',
                       'strongcorr_indep_first','strongcorr_indep_second',
                       'strongcorr_indep_group','strongcorr_corr_tree')

tree.input.options <- list('strongcorr_indep_all' = rbind(c(1,1,1,1),c(1,2,3,4)),
                           'strongcorr_corr_single' = rbind(c(1,1,1,1),c(1,2,3,4)),
                           'strongcorr_indep_first' = rbind(c(1,1,1,1),c(1,2,3,3),c(1,2,3,4)),
                           'strongcorr_indep_second' = rbind(c(1,1,1,1),c(1,1,2,3),c(1,2,3,4)),
                           'strongcorr_indep_group' = rbind(c(1,1,1,1),c(1,1,2,2),c(1,2,3,4)), 
                           'strongcorr_corr_tree' = rbind(c(1,1,1,1),c(1,1,2,2),c(1,2,3,4)))
p.matrix.options <- list('strongcorr_indep_all' = rbind(c(rep(1,4)),rep(0.1,4)),
                         'strongcorr_corr_single' = rbind(c(rep(0.1/0.9,4)),rep(0.9,4)),
                         'strongcorr_indep_first' = rbind(c(rep(1,4)),c(0.1,0.1,rep(0.1/0.9,2)),c(1,1,rep(0.9,2))),
                         'strongcorr_indep_second' = rbind(c(rep(1,4)),c(rep(0.1/0.9,2),0.1,0.1),c(rep(0.9,2),1,1)),
                         'strongcorr_indep_group' = rbind(c(rep(1,4)),c(rep(0.1/0.9,4)),rep(0.9,4)), 
                         'strongcorr_corr_tree' = rbind(c(rep(0.2,4)),c(rep(0.1/0.18,4)),rep(0.9,4)))
...
write.csv(round((auc_rocr_all),digits = 3), 
          file = paste0(data_path,'/strong_corr_auc_rocr_all','.csv'))
write.csv(round((auc_pr_all),digits = 3), 
          file = paste0(data_path,'/strong_corr_auc_pr_all','.csv'))
...
#  print( mcc.tmp)
  ...
  #           file = paste0(output_path,'/strong_corr_mcc_',ss.ix,'.csv'))
  # write.csv(round(fdr.tmp,digits = 3), 
  #           file = paste0(output_path,'/strong_corr_fdr_',ss.ix,'.csv'))
}
write.csv(round((summary_mcc),digits = 3), file = paste0(data_path,'/strong_corr_mcc_all','.csv'))
write.csv(round((summary_fdr),digits = 3), file = paste0(data_path,'/strong_corr_fdr_all','.csv'))
...
de.pattern <- c("strongcorr_indep_all", "strongcorr_corr_single",
                "strongcorr_indep_first", "strongcorr_indep_second",
                "strongcorr_indep_group","strongcorr_corr_tree")
# fig4 
### ROC curve for strong correlation
png(paste0(data_path,'/fig4_de_pattern_ROC_strong_corr.png'),
...
png(paste0(data_path,'/fig4_de_pattern_FDRcontrol_strong_corr.png'),
    width= 15.6, height = 18.6, units = 'in', res = 480)
...
### TDR curve
png(paste0(data_path,'/fig4_de_pattern_TDR_strong_corr.png'),
    width= 15.6, height = 18.6, units = 'in', res = 480)
...
### PR curve 
png(paste0(data_path,'/fig4_de_pattern_PR_strong_corr.png'),
    width= 15.6, height = 18.6, units = 'in', res = 480)
...
png(paste0(data_path,'/fig4_de_pattern_MCC_strong_corr.png'),
    width= 15.6, height = 18.6, units = 'in', res = 480)
par(mfrow=c( length(de.pattern), (cell.num + 1) ), omi = c(0.3,0,0.3,0.3))
