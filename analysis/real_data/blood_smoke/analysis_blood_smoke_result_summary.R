...
output.res <- cbind(liu.res$cedar$toast_res$Lym[DMCT_name_v,'fdr'],
                    ...
                    liu.res$cssam$`Lym:1`[DMCT_name_v,'FDR'],
                    liu.res$cssam$`Mye:1`[DMCT_name_v,'FDR'],
                    liu.res$celldmc$coe$Lym[DMCT_name_v,'adjP'],
                    liu.res$celldmc$coe$Mye[DMCT_name_v,'adjP'],
                    1 - liu.res$cedar$tree_res$single$pp[DMCT_name_v,'Lym'],
                    1 - liu.res$cedar$tree_res$single$pp[DMCT_name_v,'Mye'])

methods <- c('TOAST','TCA','csSAM','CellDMC','CeDAR-S')
...
output.res <- cbind(hannum.res$cedar$toast_res$Lym[DMCT_name_v,'fdr'],
                    ...
                    hannum.res$cssam$`Lym:2`[DMCT_name_v,'FDR'],
                    hannum.res$cssam$`Mye:2`[DMCT_name_v,'FDR'],
                    hannum.res$celldmc$coe$Lym[DMCT_name_v,'adjP'],
                    hannum.res$celldmc$coe$Mye[DMCT_name_v,'adjP'],
...
fig.output <- ggarrange(fig.1, fig.2, ncol = 1, labels = c("a", "b"))
fig.output

png(paste0(output.path, '/fig_blood_smoke_6_methods.png'),
    units='in', res=480,height=14, width = 10)
print(fig.output)
dev.off()
